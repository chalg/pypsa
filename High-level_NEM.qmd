...
The workflow implemented in this Quarto document encompasses the following key steps:  
  ...
  - **Optimisation and Scenario Analysis:** Solving the baseline model using linear optimisation to determine the optimal dispatch of resources, and extending the analysis to a future scenario with an aggressive increase in variable renewable energy and associated reduction in thermal generation.   
...
Also, consider the relationship between gas generation capacity and demand in SA1, and similarly, between hydro capacity and demand in TAS1. As a result, these regions tend to experience unserved energy later than the others.
...
The `6.1_6xVreTransitionZeroCoal` scenario achieves a 75% reduction in gas generation^[compared to 10,919 GWh - recent 12-month Open Electricity '1Y-Simplified' view. [Source:](https://explore.openelectricity.org.au/energy/nem/?range=1y&interval=1w&view=discrete-time&group=Simplified)] however only results in 48.4 GW of battery capacity - equivalent to ~1.42 times the 34 GW projected for 2049–50 in the AEMO ISP *Step Change* scenario for residential and commercial batteries^[2024 ISP, section 4.2, page 50]. Consequently, this scenario would likely be significantly more economical from a cost perspective compared to `8.3_VreCurtailReview`.
...
I spent several hours adjusting scripts and running all of these scenarios at a 30-minute resolution, and I observed a greater amount of unserved energy. Only 3 out of the 20 scenarios resulted in no unserved energy - and in each of those three cases, gas generation capacity was not reduced.

The three scenarios with no unserved energy were:  
  - `0_2024_baseline`  
  - `1_BalancedTransition`  
  - `8.0_6xVre&BatteryZeroCoal`  

This demonstrates that while hourly resolution is a helpful indicator, it is not fully reliable for evaluating the dispatch profile.

<div style="text-align: center;">
![Unserved Energy in the `8.3_VreCurtailReview` scenario at 30 minute resolution](images/unserved-energy-heatmap.png)
</div>
...
Potential future analysis would also incorporate:  
  ...
  -  as indicated above, more granular time resolution (possibly for May/June only due to increased computation)  
...
::: {.callout-note}
*30mins* resolution was added as a drop-down option to view a modified subset of the hourly scenarios at 30-minute resolution.

The modifications were primarily to reduce increased unserved energy at the more granular temporal resolution. 

I've deliberately left `8.1_6xVre&Battery_30min` to show an example of a small amount of remaining unserved energy on the evening of Jun 13 in VIC1.
:::

Like this webpage, the Streamlit app is better viewed on a large screen.
