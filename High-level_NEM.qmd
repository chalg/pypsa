...
def plot_peak_demand_vs_capacity(network, stack_carrier=False, relative=False, sort_by="Max Demand"):
    ...
    if stack_carrier:
        ...
        # Sort carriers by total capacity for consistent stacking
        carriers = list(capacity_by_bus_carrier.columns)
        carrier_totals = {c: capacity_by_bus_carrier[c].sum() for c in carriers}
        carriers = sorted(carriers, key=lambda x: carrier_totals[x], reverse=True)
...
def plot_total_demand_vs_generation(network, stack_carrier=False, relative=False):
    ...
    if stack_carrier:
        ...
        # Sort carriers by total generation for consistent stacking
        carrier_totals = {c: relative_generation[c].sum() for c in carriers}
        carriers = sorted(carriers, key=lambda x: carrier_totals[x], reverse=True)
...
The model applies relatively harsh curtailment to solar and wind, but particularly rooftop solar. This is a result of curtailment already being incorporated into utility-scale solar availability (capacity) factors, which I've sourced and then calculated from AEMO’s NEMWEB dispatch data. 
