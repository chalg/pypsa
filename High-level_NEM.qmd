...
This Quarto document presents the development of a high-level baseline model for the NEM power system using [PyPSA (Python for Power System Analysis)](https://pypsa.readthedocs.io/en/latest/), an open-source framework for simulating and optimizing modern energy systems. The model, referred to as High-level_NEM, is a 5-node model (one bus per region) based on actual 2024 generation and demand profiles at 30-minute resolution. A comparison of the 2024 baseline model with a future scenario is presented in this document.
...
Baseline 2024 model:  
  ...
  - Marginal price of wind, utility-scale solar, rooftop solar and hydro^[The marginal price for hydro is $8.58 according to the 2024 ISP Inputs and Assumptions workbook; however, I wanted to prevent the solver from underutilising hydro.] set to zero  
...
[Github Repository](https://github.com/chalg/pypsa/tree/main)      
...
Note: one of the scenarios have unserved energy (bold, red font), which tends to show up in NSW and VIC before other regions.
...
The `8.3_VreCurtailReview` scenario results in a 21.6% curtailment rate for wind and utility-scale solar, which is quite close to the 20% level projected by the ISP for 2050 (2024 ISP, section 4.3, page 52). However, since some curtailment is already embedded in the source data for this scenario, the effective curtailment rate is significantly higher.
...
Potential future analysis could incorporate:  
  ...
  -  remove the built-in curtailment from the data or apply an alternative measurement approach, such as using synthetic wind and solar traces.   
...
New South Wales is attracting the most breaches at just over 37% of observations breached.
